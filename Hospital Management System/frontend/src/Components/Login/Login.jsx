...
const Login = () => {
    //useState to hook to store inputs 
    ...
    const loginUser  =(e)=>{
        ...
        Axios.post('http://localhost:3002/login',
        {
            ...
            if(response.data.message || loginUserName == '' || loginPassword == ''){
                navigateTo('/')
                setLoginStauts(`Credentials Don't Exist!`)
            }
            else{
                navigateTo('/dashboard') //if the data are correct it navigate to dashboard
            }
        })
    }
    ...
    useEffect(()=>{
        if(loginStatus !== ''){
            ...
            setTimeout(()=>{
                setStatusHolder('message')//hide message after 4 sec
            },8000);
        }
    }, [loginStatus])
